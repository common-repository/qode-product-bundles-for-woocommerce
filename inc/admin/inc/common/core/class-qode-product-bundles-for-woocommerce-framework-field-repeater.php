...
class Qode_Product_Bundles_For_WooCommerce_Framework_Field_Repeater implements Qode_Product_Bundles_For_WooCommerce_Framework_Tree_Interface, Qode_Product_Bundles_For_WooCommerce_Framework_Child_Interface {
	...
	public function __construct( $params ) {
		...
		$this->button_text = isset( $params['button_text'] ) ? $params['button_text'] : esc_html__( '+ Add New Item', 'qode-product-bundles-for-woocommerce' );
	...
	public function add_repeater_inner_element( $params ) {
		if ( isset( $params['name'] ) && ! empty( $params['name'] ) ) {
			...
			$field          = new Qode_Product_Bundles_For_WooCommerce_Framework_Field_Repeater_Inner( $params );
	...
	public function add_field_element( $params ) {
		...
		if ( isset( $params['name'] ) && ! empty( $params['name'] ) ) {
			...
			$field                   = new Qode_Product_Bundles_For_WooCommerce_Framework_Field_Mapper( $params );
			$this->add_child( $field );
	...
	public function render() {
		$this->value = qode_product_bundles_for_woocommerce_framework_get_option_value( $this->scope, $this->type, $this->name ); ?>

		<div <?php qode_product_bundles_for_woocommerce_class_attribute( $this->params['wrapper_classes'] ); ?> <?php qode_product_bundles_for_woocommerce_inline_attrs( $this->params['wrapper_data'] ); ?>>
					...
					<div class="qodef-repeater-wrapper-main col-12 <?php echo esc_attr( $this->params['sortable_class'] ); ?>" data-template="<?php echo esc_attr( str_replace( '_', '-', $this->name ) ); ?>">
							...
							foreach ( $this->value as $row ) {
								...
								<div class="qodef-repeater-fields-holder clearfix" data-index="<?php echo esc_attr( $counter ); ?>">
									<h4 class="qodef-repeater-fields-label">
										<?php echo esc_html__( 'Item ', 'qode-product-bundles-for-woocommerce' ) . esc_html( $counter + 1 ); ?>
									</h4>
									...
									<div class="qodef-repeater-actions-holder">
										<div class="qodef-repeater-toggle">
											<a class="qodef-row-toggle" href="#">
												<?php qode_product_bundles_for_woocommerce_framework_svg_icon( 'toggle' ); ?>
											</a>
										</div>
										<div class="qodef-repeater-sort">
											<?php qode_product_bundles_for_woocommerce_framework_svg_icon( 'expand' ); ?>
										</div>
										<div class="qodef-repeater-remove">
											<a class="qodef-clone-remove" href="#" rel="noopener noreferrer"><?php qode_product_bundles_for_woocommerce_framework_svg_icon( 'trash' ); ?></a>
										</div>
									</div>
								</div>
								<?php
						...
						<script type="text/html" class="qodef-repeater-template" id="tmpl-qodef-repeater-template-<?php echo esc_attr( str_replace( '_', '-', $this->name ) ); ?>">
							<div class="qodef-repeater-fields-holder <?php echo esc_attr( $this->params['sortable_class'] ); ?> clearfix" data-index="{{{ data.rowIndex }}}">
								<h4 class="qodef-repeater-fields-label">
									<?php echo esc_html__( 'Item ', 'qode-product-bundles-for-woocommerce' ); ?> {{{ data.rowIndex+1 }}}
								</h4>
								<div class="qodef-repeater-fields">
									<?php
									foreach ( $this->children as $child ) {
						...
						<?php
						foreach ( $this->children as $child ) {
							if ( $child instanceof Qode_Product_Bundles_For_WooCommerce_Framework_Field_Repeater_Inner ) {
